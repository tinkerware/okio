...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
  ...
  public Buffer copyTo(Buffer out, long offset, long byteCount) {
    ...
    while (byteCount > 0) {
      ...
      if (target.limit == Segment.SIZE) {
        target = target.push(currentPool().take());
      }
  ...
  Segment writableSegment(int minimumCapacity) {
    ...
    if (head == null) {
      head = currentPool().take(); // Acquire a first segment.
      return head.next = head.prev = head;
    }

    Segment tail = head.prev;
    if (tail.limit + minimumCapacity > Segment.SIZE) {
      tail = tail.push(currentPool().take()); // Append a new empty segment to fill up.
    }
    return tail;
  }
